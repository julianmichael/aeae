...
class NliClassifier(Model):
    ...
    def make_output_human_readable(
        ...
        for prediction in predictions_list:
            ...
            label_vocab = self.vocab.get_index_to_token_vocabulary(self._label_namespace)

            label_idx = prediction.argmax(dim=-1).item()
            label_str = label_vocab.get(label_idx, str(label_idx))
            classes.append(label_str)

            label_dist = {
                self.vocab.get_token_from_index(i, self._label_namespace): prob
                for i, prob in enumerate(prediction)
            }
            class_probs.append(label_dist)

        output_dict["max_label"] = classes
        output_dict["label_probs"] = class_probs
        # @julianmichael: can add this back for debugging if necessary
        # tokens = []
